...
internal/
  ...
  format/         six built-in formatters + CmdFormatter (user-defined, KV-injected)
  ...
  pipe/           client-side output relay, hybrid binary/text reader
  pipeline/       per-pane line processing (filter chain + KV injection, sinks)
  router/         TargetID → Pipeline routing + BookmarkStore per pane
  selftest/       11 in-process smoke checks
  server/         HTTP/WebSocket daemon, REST API, embedded SPA, /admin page
...
docs/
  ...
  manual.md        complete user manual (29 sections)
...
## Adding a user-defined formatter at runtime

No code needed. Register from the CLI:

```sh
rdw formatter register myfmt './myfmt.sh'
```
...
The script receives pane scrollback on stdin and writes HTML to stdout. KV is injected as environment variables.

## New commands added since initial implementation

| Command | Description |
| --- | --- |
| `rdw send FILE --id ID` | Send a file to a pane (auto-detects type) |
| `rdw pane rename ID TITLE` | Set pane display title |
| `rdw formatter list/register/unregister` | Manage user formatters |
| `rdw status [pane ID]` | Server introspection snapshot |
| `rdw cycle start/stop/status` | Focus cycle automation |
